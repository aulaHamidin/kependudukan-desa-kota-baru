<x-card :padding="false">
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h3 class="text-lg font-semibold text-gray-800">Daftar RT</h3>
                <p class="text-sm text-gray-500">Total {{ $rts->count() }} data RT</p>
            </div>
            @can('create', [\App\Models\Rt::class, auth()->user()->rw_id ?? 0])
                <div>
                    <button type="button" class="btn btn-primary" x-on:click="$dispatch('open-modal', 'create-rt')">
    ...
    @if ($rws->count() > 1)
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50" x-data="rtTableFilter()" x-init="init()">
            ...
            <div class="flex items-end gap-3">
                <div class="flex-1">
                    <label class="block text-xs font-medium text-gray-700 mb-1.5">RW</label>
                    <select x-model="filters.rw" class="form-select-custom w-full">
                        <option value="">Semua RW</option>
                        @foreach ($rws as $rw)
                            @php
                                $rwLabel = 'RW' . str_pad($rw->nomor_rw ?? '', 3, '0', STR_PAD_LEFT);
                            @endphp
                            <option value="{{ $rwLabel }}">{{ $rwLabel }} - {{ $rw->desa->nama ?? '-' }}
                            </option>
                        @endforeach
                    </select>
                </div>
    ...
    <x-data-table :datatable="true" :datatableOptions="[
        ...
        <x-slot name="head">
            <tr>
                ...
                <x-table-header>RW</x-table-header>
                <x-table-header>Nomor RT</x-table-header>
        ...
        @forelse ($rts as $item)
            @php
                ...
                $rwLabel = 'RW' . str_pad($item->rw->nomor_rw ?? '', 3, '0', STR_PAD_LEFT);
            @endphp
            <tr class="table-row-hover">
                <x-table-cell>{{ $item->rw->desa->nama ?? '-' }}</x-table-cell>
                <x-table-cell>{{ $rwLabel }}</x-table-cell>
                    ...
                    <div class="flex items-center justify-center gap-1">
                        @can('view', $item)
                            <button type="button" class="btn-action btn-action-view" title="Lihat Detail"
                                x-on:click="$dispatch('open-drawer', 'rt-detail-{{ $item->id }}')">
                        ...
                        @can('delete', $item)
                            <button type="button" class="btn-action btn-action-delete" title="Hapus"
                                x-on:click="$dispatch('open-modal', 'delete-rt-{{ $item->id }}')">
        ...
        @empty
            <tr>
                <td colspan="6">
                    <x-empty-state title="Belum Ada Data RT" description="Data RT belum tersedia dalam sistem."
                        icon="empty">
                        @can('create', [\App\Models\Rt::class, auth()->user()->rw_id ?? 0])
                            <button type="button" class="btn btn-primary mt-4"
                                x-on:click="$dispatch('open-modal', 'create-rt')">
                                <x-button-icon icon="plus" />
                                Tambah RT
                            </button>
                        @endcan
