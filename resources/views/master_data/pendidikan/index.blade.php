<x-app-layout>
    <x-slot name="title">Data Pendidikan</x-slot>

    <x-slot name="breadcrumb">
        <x-breadcrumb :items="[['label' => 'Master Referensi'], ['label' => 'Pendidikan']]" />
    </x-slot>

    <x-slot name="header">
        <x-page-header title="Data Pendidikan" subtitle="Kelola data master pendidikan dalam sistem">
        </x-page-header>
    </x-slot>
    ...
    <div class="mb-6 rounded-lg border border-gray-200 bg-gray-50 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="text-base font-semibold text-gray-800">Data pendidikan bersifat tetap
                </h3>
                <p class="text-sm text-gray-500 mt-1">
                    Data pendidikan berasal dari seed dan tidak bisa ditambah, diubah, atau dihapus. Silakan hubungi
    ...
    <x-card :padding="false">
        <x-slot name="header">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800">Daftar Pendidikan</h3>
                    <p class="text-sm text-gray-500">Total {{ $items->count() }} data pendidikan
                    </p>
                </div>
            </div>
        </x-slot>
        ...
        <x-data-table :datatable="true" :datatableOptions="[
            ...
            <x-slot name="filters">
                <x-table.action-bar datatableSearchFor="pendidikanTable" :compact="true" />
            </x-slot>
            ...
            @empty
                <tr>
                    <td colspan="5">
                        <x-empty-state title="Belum Ada Data Pendidikan"
                            description="Data pendidikan belum tersedia dalam sistem." icon="empty" />
                    </td>
                </tr>
            @endforelse
        </x-data-table>
