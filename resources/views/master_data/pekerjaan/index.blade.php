<x-app-layout>
    <x-slot name="title">Data Pekerjaan</x-slot>

    <x-slot name="breadcrumb">
        <x-breadcrumb :items="[['label' => 'Master Referensi'], ['label' => 'Pekerjaan']]" />
    </x-slot>

    <x-slot name="header">
        <x-page-header title="Data Pekerjaan" subtitle="Kelola data master pekerjaan dalam sistem">
        </x-page-header>
    </x-slot>
    ...
    <div class="mb-6 rounded-lg border border-gray-200 bg-gray-50 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="text-base font-semibold text-gray-800">Data pekerjaan bersifat tetap
                </h3>
                <p class="text-sm text-gray-500 mt-1">
                    Data pekerjaan berasal dari seed dan tidak bisa ditambah, diubah, atau dihapus. Silakan hubungi
    ...
    <x-card :padding="false">
        <x-slot name="header">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Total {{ $items->count() }} data pekerjaan</p>
                </div>
            </div>
        </x-slot>
        ...
        <x-data-table :datatable="true" :datatableOptions="[
            ...
            <x-slot name="filters">
                <x-table.action-bar datatableSearchFor="pekerjaanTable" :compact="true" />
            </x-slot>
            ...
            @empty
                <tr>
                    <td colspan="5">
                        <x-empty-state title="Belum Ada Data Pekerjaan"
                            description="Data pekerjaan belum tersedia dalam sistem." icon="empty" />
                    </td>
                </tr>
            @endforelse
        </x-data-table>
