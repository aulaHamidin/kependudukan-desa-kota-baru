<x-app-layout>
    <x-slot name="title">Data Event Type</x-slot>

    <x-slot name="breadcrumb">
        <x-breadcrumb :items="[['label' => 'Master Referensi'], ['label' => 'Event Type']]" />
    </x-slot>

    <x-slot name="header">
        <x-page-header title="Data Event Type" subtitle="Kelola data master jenis peristiwa dalam sistem">
        </x-page-header>
    </x-slot>
    ...
    <div class="mb-6 rounded-lg border border-gray-200 bg-gray-50 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="text-base font-semibold text-gray-800">Data event type bersifat tetap
                </h3>
                <p class="text-sm text-gray-500 mt-1">
                    Data event type berasal dari seed dan tidak bisa ditambah, diubah, atau dihapus. Silakan hubungi
    ...
    <x-card :padding="false">
        <x-slot name="header">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800">Daftar Event Type</h3>
                    <p class="text-sm text-gray-500">Total {{ $items->count() }} data event type
                    </p>
                </div>
            </div>
        </x-slot>
        ...
        <x-data-table :datatable="true" :datatableOptions="[
            ...
            <x-slot name="filters">
                <x-table.action-bar datatableSearchFor="eventTypeTable" :compact="true" />
            </x-slot>
            ...
            <x-slot name="head">
                <tr>
                    ...
                    <x-table-header>Detail Wajib</x-table-header>
                    <x-table-header>Status</x-table-header>
                    <x-table-header>Jumlah Event</x-table-header>
                </tr>
            </x-slot>
            ...
            @empty
                <tr>
                    <td colspan="5">
                        <x-empty-state title="Belum Ada Data Event Type"
                            description="Data event type belum tersedia dalam sistem." icon="empty" />
                    </td>
                </tr>
            @endforelse
        </x-data-table>
