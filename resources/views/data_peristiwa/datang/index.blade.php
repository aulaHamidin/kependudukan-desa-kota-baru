{{-- Data Peristiwa - Datang Index --}}
<x-app-layout>
    <x-slot name="breadcrumb">
        <x-breadcrumb :items="[['label' => 'Data Peristiwa', 'url' => '#'], ['label' => 'Datang']]" />
    </x-slot>

    <x-slot name="header">
        <x-page-header title="Event Datang" subtitle="Kelola data penduduk yang baru datang">
            <x-slot name="actions">
                @can('create', App\Models\Event::class)
                    <x-button variant="primary" icon="plus" :href="route('events.datang.create')">
                        Tambah Event
                    </x-button>
                @endcan
            </x-slot>
    ...
    <x-card class="mb-6">
        <div class="p-5">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-sm font-semibold text-gray-700">Filter Event Datang</h3>
            </div>

            <form method="GET" action="{{ route('events.datang.index') }}">
                ...
                <div class="flex gap-2 mt-4">
                    ...
                    @if (request()->anyFilled(['search', 'status_data', 'start_date', 'end_date']))
                        <a href="{{ route('events.datang.index') }}"
    ...
    <x-data-table class="min-w-full" :searchable="false">
        <x-slot name="head">
            <tr>
                ...
                <x-table-header>Jenis Kedatangan</x-table-header>
            ...
            <tr class="table-row-hover">
                ...
                <x-table-cell class="whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end gap-2">
                        @can('view', $event)
                            <x-button variant="secondary" size="sm" :href="route('events.datang.show', $event)">
                                Detail
                            </x-button>
                        @endcan
                        @can('update', $event)
                            <x-button variant="warning" size="sm" :href="route('events.datang.edit', $event)">
                                Edit
                            </x-button>
                        @endcan
                        @can('delete', $event)
                            <x-delete-confirm :action="route('events.datang.destroy', $event)" title="Hapus Event Datang?"
                                text="Data event datang ini akan dihapus permanen.">
        ...
        @empty
            <tr>
                <td colspan="6" class="px-6 py-12">
                    <x-empty-state title="Belum ada event datang"
                        description="Klik tombol 'Tambah Event' untuk mencatat event penduduk datang baru." />
                </td>
            </tr>
        @endforelse
