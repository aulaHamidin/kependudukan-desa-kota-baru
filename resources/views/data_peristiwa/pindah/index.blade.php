{{-- Data Peristiwa - Pindah Index --}}
<x-app-layout>
    <x-slot name="breadcrumb">
        <x-breadcrumb :items="[['label' => 'Data Peristiwa', 'url' => '#'], ['label' => 'Pindah']]" />
    </x-slot>

    <x-slot name="header">
        <x-page-header title="Event Pindah" subtitle="Kelola data peristiwa pindah domisili">
            <x-slot name="actions">
                @can('create', App\Models\Event::class)
                    <x-button variant="primary" icon="plus" :href="route('events.pindah.create')">
                        Tambah Event
                    </x-button>
                @endcan
            </x-slot>
    ...
    <x-card class="mb-6">
        <div class="p-5">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-sm font-semibold text-gray-700">Filter Event Pindah</h3>
            </div>

            <form method="GET" action="{{ route('events.pindah.index') }}">
                ...
                <div class="flex gap-2 mt-4">
                    ...
                    @if (request()->anyFilled(['search', 'status_data', 'start_date', 'end_date']))
                        <a href="{{ route('events.pindah.index') }}"
    ...
    <x-data-table class="min-w-full" :searchable="false">
        <x-slot name="head">
            <tr>
                ...
                <x-table-header>Nama Penduduk</x-table-header>
                <x-table-header>Alamat Tujuan</x-table-header>
        ...
        @forelse($events as $event)
            <tr class="table-row-hover">
                ...
                <x-table-cell class="text-sm text-gray-700 max-w-xs truncate">
                ...
                <x-table-cell class="whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end gap-2">
                        @can('view', $event)
                            <x-button variant="secondary" size="sm" :href="route('events.pindah.show', $event)">
                                Detail
                            </x-button>
                        @endcan
                        @can('update', $event)
                            <x-button variant="warning" size="sm" :href="route('events.pindah.edit', $event)">
                                Edit
                            </x-button>
                        @endcan
                        @can('delete', $event)
                            <x-delete-confirm :action="route('events.pindah.destroy', $event)" title="Hapus Event Pindah?"
                                text="Data event pindah ini akan dihapus permanen.">
        ...
        @empty
            <tr>
                <td colspan="6" class="px-6 py-12">
                    <x-empty-state title="Belum ada event pindah"
                        description="Klik tombol 'Tambah Event' untuk mencatat event pindah baru." />
                </td>
            </tr>
        @endforelse
